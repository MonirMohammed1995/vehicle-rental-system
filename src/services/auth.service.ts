...
export class AuthService {
  static async signup(data: { name: string; email: string; password: string; phone: string; role?: string }) {
    ...
    if (ex.rows.length) throw new Error("Email already in use");
  ...
  static async signin(emailIn: string, password: string) {
    ...
    const r = await pool.query("SELECT id,name,email,password,phone,role FROM users WHERE email=$1", [email]);
